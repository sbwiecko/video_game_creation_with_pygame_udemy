...
pick_up_sound = pygame.mixer.Sound("./snake_assets/pick_up_sound.wav")
...
# the list will contain all the cumulative rect
body_coords = []
...
while running:
    ...
    # insert the coordinates of the current head rect and remove the tail rect
    body_coords.insert(0, head_coord)
    body_coords.pop()

    #THEN update the x,y position of the snakes head and make a new coordinate
    head_x += snake_dx
    head_y += snake_dy
    head_coord = (head_x, head_y, SNAKE_SIZE, SNAKE_SIZE)

    #Check for game over; beware the order of the coordinate update and intra-collision !
    ...
    # no detection of intra-collision but rather if the head coordinates are already in the body coordinates
        display_surface.blit(game_over_text, game_over_rect)
        display_surface.blit(continue_text, continue_rect)
        pygame.display.update()

        pygame.time.wait(1000) # wait to avoid player pushing down a key at the game over
    ...
    #Blit assets
    # display all the rect of the body list
